...
public class AndroidMockTest extends TestCase {
  private List<String> notForwardedMethods = new ArrayList<String> (Arrays.asList(new String[] {
  ...
  private CtMethod[] getForwardedMethods() throws NotFoundException {
    List<CtMethod> methods = new ArrayList<CtMethod>(Arrays.asList(
        getAndroidMockCtClass().getDeclaredMethods()));
  ...
  public void testIsUnboxableToPrimitiveAllWideningPrimitives() {
    Object[] testValues = new Object[] {
        new Byte((byte)42), new Short((short)42), new Integer(42), new Long(42L), new Float(42.0f),
        new Double(42.0), new Character('a'), Boolean.TRUE
    };
    ...
    boolean[] booleanExpected = new boolean[] {
        false, false, false, false, false, false, false, true};
    
    for (int i = 0; i < testValues.length; ++i) {
      assertEquals("Convert byte from " + testValues[i].getClass(), byteExpected[i], 
          AndroidMock.isUnboxableToPrimitive(Byte.TYPE, testValues[i], false));
      assertEquals("Convert short from " + testValues[i].getClass(), shortExpected[i],
          AndroidMock.isUnboxableToPrimitive(Short.TYPE, testValues[i], false));
      assertEquals("Convert int from " + testValues[i].getClass(), intExpected[i],
          AndroidMock.isUnboxableToPrimitive(Integer.TYPE, testValues[i], false));
      assertEquals("Convert long from " + testValues[i].getClass(), longExpected[i],
          AndroidMock.isUnboxableToPrimitive(Long.TYPE, testValues[i], false));
      assertEquals("Convert float from " + testValues[i].getClass(), floatExpected[i],
          AndroidMock.isUnboxableToPrimitive(Float.TYPE, testValues[i], false));
      ...
      assertEquals("Convert char from " + testValues[i].getClass(), charExpected[i],
          AndroidMock.isUnboxableToPrimitive(Character.TYPE, testValues[i], false));
  ...
  public void testCreateMock() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(Vector.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
    // in a separate VM instance with the classes already available in a jar file.
    for (CtClass clazz : mockClasses) {
      try {
        clazz.toClass();
      } catch (CannotCompileException e) {
        // This is only because of the hack of calling toClass() to force the
        // ClassLoader to find the classes. This won't ever happen in real life.
      }
    }
  ...
  public void testCreateMockUsingParameters() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(
  ...
  public void testCreateMockUsingProtectedConstructors() throws ClassNotFoundException {
    ...
    // Just don't explode
    Calendar newMockCalendar = AndroidMock.createMock(Calendar.class, new SimpleTimeZone(1, "GMT"),
        Locale.UK);
  }

  public void testCreateMockUsingCastableParameters() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(
        SimpleTimeZone.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
  ...
  public void testCreateMockUsingUnusableParameters() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(
        SimpleTimeZone.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
  ...
  public void testCreateMockUsingInterface() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(Map.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
    // in a separate VM instance with the classes already available in a jar file.
    for (CtClass clazz : mockClasses) {
  ...
  public void testCreateMockUsingClass() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(Vector.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
    // in a separate VM instance with the classes already available in a jar file.
  ...
  public void testCreateNiceMock() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(Vector.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
    // in a separate VM instance with the classes already available in a jar file.
  ...
  public void testCreateNiceMockUsingUnusableParameters() throws ClassNotFoundException {
    ...
    try {
      SimpleTimeZone mockTimeZone = AndroidMock.createNiceMock(
          SimpleTimeZone.class, 0, new Object());
      fail("Excepted an IllegalArgumentException for incorrect constructor parameters");
    } catch (IllegalArgumentException e) {
      // Expected
    }
  ...
  public void testCreateNiceMockUsingParameters() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(
        SimpleTimeZone.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
  ...
  public void testCreateNiceMockUsingInterface() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(Map.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
    // in a separate VM instance with the classes already available in a jar file.
  ...
  public void testCreateNiceMockUsingClass() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(Vector.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
    // in a separate VM instance with the classes already available in a jar file.
  ...
  public void testCreateStrictMock() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(Vector.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
    // in a separate VM instance with the classes already available in a jar file.
  ...
  public void testCreateStrictMockUsingUnusableParameters() throws ClassNotFoundException {
    ...
    try {
      SimpleTimeZone mockTimeZone = AndroidMock.createStrictMock(
          SimpleTimeZone.class, 0, new Object());
      fail("Excepted an IllegalArgumentException for incorrect constructor parameters");
    } catch (IllegalArgumentException e) {
      // Expected
  ...
  public void testCreateStrictMockUsingParameters() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(
        SimpleTimeZone.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
  ...
  public void testCreateStrictMockUsingInterface() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(Map.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
    // in a separate VM instance with the classes already available in a jar file.
  ...
  public void testCreateStrictMockUsingClass() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(Vector.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
    // in a separate VM instance with the classes already available in a jar file.
  ...
  public void testCreateMockConstructorDoesWorkOnAllReturnTypes() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(
        ClassDoesWorkInConstructor.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
    // in a separate VM instance with the classes already available in a jar file.
    for (CtClass clazz : mockClasses) {
  ...
  public void testAllForwardedMethods() throws CannotCompileException, NotFoundException {
    for (CtMethod method : getForwardedMethods()) {
      ...
      // CtMethod.instrument Causes every instruction in the method to be inspected, and passed to
      // the MethodVerifier callback (extends javassist.expr.ExprEditor). We want to verify that
      // the expected EasyMock method is called at least once in each AndroidMock method.
      method.instrument(verifier);
      assertTrue(method.getLongName() + " not called.", verifier.expectedMethodCalled());
    }
  }

  public void testCheckOrder() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(Vector.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
    // in a separate VM instance with the classes already available in a jar file.
  ...
  public void testVerify() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(Vector.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
    // in a separate VM instance with the classes already available in a jar file.
  ...
  public void testResetToStrict() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(Vector.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
    // in a separate VM instance with the classes already available in a jar file.
  ...
  public void testResetToDefault() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(Vector.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
    // in a separate VM instance with the classes already available in a jar file.
  ...
  public void testResetToNice() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(Vector.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
    // in a separate VM instance with the classes already available in a jar file.
  ...
  public void testReset() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(Vector.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
    // in a separate VM instance with the classes already available in a jar file.
  ...
  public void testReplay() throws ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(Vector.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
    // in a separate VM instance with the classes already available in a jar file.
  ...
  public void testAnd() {
    Map<?, ?> mockMap = AndroidMock.createMock(Map.class);
    AndroidMock.expect(mockMap.get(AndroidMock.and(AndroidMock.eq(true),
        AndroidMock.eq(true)))).andReturn(null);
    AndroidMock.expect(mockMap.get(AndroidMock.and(AndroidMock.eq((byte) 0),
        AndroidMock.eq((byte) 0)))).andReturn(null);
    AndroidMock.expect(mockMap.get(AndroidMock.and(AndroidMock.eq((short) 0),
        AndroidMock.eq((short) 0)))).andReturn(null);
    AndroidMock.expect(mockMap.get(AndroidMock.and(AndroidMock.eq(0),
        AndroidMock.eq(0)))).andReturn(null);
    AndroidMock.expect(mockMap.get(AndroidMock.and(AndroidMock.eq(0L),
        AndroidMock.eq(0L)))).andReturn(null);
    AndroidMock.expect(mockMap.get(AndroidMock.and(AndroidMock.eq(0.0),
        AndroidMock.eq(0.0)))).andReturn(null);
    AndroidMock.expect(mockMap.get(AndroidMock.and(AndroidMock.eq(0.0f),
        AndroidMock.eq(0.0f)))).andReturn(null);
    AndroidMock.expect(mockMap.get(AndroidMock.and(AndroidMock.eq("hi"),
        AndroidMock.eq("hi")))).andReturn(null);
    AndroidMock.expect(mockMap.get(AndroidMock.and(AndroidMock.eq('a'),
        AndroidMock.eq('a')))).andReturn(null);
    AndroidMock.replay(mockMap);
  }

  public void testOr() {
    Map<?, ?> mockMap = AndroidMock.createMock(Map.class);
    AndroidMock.expect(mockMap.get(AndroidMock.or(AndroidMock.eq(true),
        AndroidMock.eq(true)))).andReturn(null);
    AndroidMock.expect(mockMap.get(AndroidMock.or(AndroidMock.eq((byte) 0),
        AndroidMock.eq((byte) 0)))).andReturn(null);
    AndroidMock.expect(mockMap.get(AndroidMock.or(AndroidMock.eq((short) 0),
        AndroidMock.eq((short) 0)))).andReturn(null);
    AndroidMock.expect(mockMap.get(AndroidMock.or(AndroidMock.eq(0),
        AndroidMock.eq(0)))).andReturn(null);
    AndroidMock.expect(mockMap.get(AndroidMock.or(AndroidMock.eq(0L),
        AndroidMock.eq(0L)))).andReturn(null);
    AndroidMock.expect(mockMap.get(AndroidMock.or(AndroidMock.eq(0.0),
        AndroidMock.eq(0.0)))).andReturn(null);
    AndroidMock.expect(mockMap.get(AndroidMock.or(AndroidMock.eq(0.0f),
        AndroidMock.eq(0.0f)))).andReturn(null);
    AndroidMock.expect(mockMap.get(AndroidMock.or(AndroidMock.eq("hi"),
        AndroidMock.eq("hi")))).andReturn(null);
    AndroidMock.expect(mockMap.get(AndroidMock.or(AndroidMock.eq('a'),
        AndroidMock.eq('a')))).andReturn(null);
    AndroidMock.replay(mockMap);
  }
  ...
  public void testCmp() {
    Map<?, ?> mockMap = AndroidMock.createMock(Map.class);
    AndroidMock.expect(mockMap.get(AndroidMock.cmp("hi", String.CASE_INSENSITIVE_ORDER,
  ...
  public void testAndThrowsOnMockedClass() throws IOException, ClassNotFoundException {
    List<CtClass> mockClasses = new AndroidMockGenerator().createMocksForClass(InputStream.class);
    // This is a hack to force the classes to be findable by the ClassLoader.
    // This never happens in a real scenario since the Android Mock is used
    // in a separate VM instance with the classes already available in a jar file.
    for (CtClass clazz : mockClasses) {
  ...
  /**
   * Used for testing that a given method on Android Mock calls the equivalent method on EasyMock,
   * to ensure that the method-wiring of Android Mock is correct.
   * 
   * @author swoodward@google.com (Stephen Woodward)
   */
